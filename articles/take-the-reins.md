...
以前、AIコーディングの原則[^19]とコードレビューの再定義[^20]について書いた。コードの責任は人間が取る。レビューはメンタルモデルの共有であるとした。どちらも、AIコーディング時代であっても変わらない原則だ。しかし原則だけでは、現場は変わらない。

AIコーディングツールが強力であることは間違いない。問題は、その力がチームの生産性向上に繋がらないことだ。AIにコードを任せたら、書く時間は減った。代わりに増えたのは、「これで本当にいいのか」と判断する時間だ。判断の基準が自分の中にしかないから、毎回ゼロから考えることになる。

足りなかったのは、手綱だ。強力な馬を御すための馬具。テスト対象を隔離して検証するためのテストハーネス。AIの能力を制限するのではなく、人間がプログラミングの複雑性を克服するために50年かけて磨いてきた技法を、AIが使えるように環境を整える。2026年のコミュニティが「ハーネスエンジニアリング」と呼び始めたのは、この環境設計の技術である。

本稿では、ハーネスとは何か、なぜ今それが必要なのか、そしてあなたが既に持っている技術がどう武器になるかを整理する。

---

![red](/images/flickr-49733608486-red.jpg)

## なぜ振り回されるのか：AIの構造的限界

### AIは「正しさ」を感じられない

人間のエンジニアは、自分が書いたコードの結果を引き受ける。動かなければ自分が直す。判断を間違えればチームに迷惑がかかる。この「結果との因果的接触」が、自然にコードの品質を引き上げる。
...
AIコーディングの原則[^19]で、ドキュメントは「書かれても読まれないもの」から、AIにコンテキストを渡すための重要な投資に変わったと書いた。ここではその先を考える。
