...
この技術がAIに対しても有効だ。理由はシンプルで、**AIも人間と同じ問題（「どこまでを考えるか」）に直面している**からだ。Bounded Contextで分割されたサービスなら、エージェントに渡すのはその境界の内側だけでいい。`Customer`は常に一つの意味を持つ。探索空間が劇的に縮む。OpenAIのCodexチームはこれを「中央で境界を強制し、局所で自律を許す」と表現した[^6]。
...
ハーネスは「意味が変わる場所」に置く。ワイヤリングハーネスのコネクタがモジュール間の接続点に置かれるのと同じだ。

### 第一の境界：人間の意図 → エージェントの入力

仕様書、ADR、AGENTS.md。人間の暗黙知をエージェントが処理できる形に翻訳する。**最もレバレッジが高い境界**である。悪い計画の1行は何百行もの悪いコードを生む。

### 第二の境界：全体 → 作業スコープ

Bounded Context、モジュール境界、依存方向の強制。エージェントの「世界」の広さを定義し、フレーム問題を管理する。

### 第三の境界：エージェントの出力 → コードベースへのマージ

ユニットテスト、CI、リンター、コードレビュー。グラウンディングの代替として「正しさ」を機械的に検証する。多くのチームはここから始めるのが現実的だろう。

### 第四の境界：セッション → セッション

進捗ファイル、git履歴、フィーチャーリスト。Anthropicが研究した長時間エージェントの引き継ぎ問題。`claude-progress.txt` がシフト交代のノートの役割を果たす[^15]。
...
[^15]: 長時間エージェントの引き継ぎ問題と二重アーキテクチャによるハーネス設計。<https://www.anthropic.com/engineering/effective-harnesses-for-long-running-agents>
