...
ここでPeter Naurの論文「Programming as Theory Building」[^5]が示唆を与えてくれる。Naurによれば、プログラムの本質はソースコードにはない。プログラマの頭の中にある「理論」、すなわちメンタルモデル(=ドメインモデル)こそが本体だ。ソースコードはメンタルモデルの不完全な記述に過ぎず、このモデルが失われたプログラムは「死んだプログラム」になる。修正はできても、なぜそう設計されたかの理解が欠落するからだ。
...
コードレビューの本質がメンタルモデルの共有にあることが見えてきた。この理解に立つと、「AIがコードを書く時代にレビューは不要」という主張の危うさがわかる。
