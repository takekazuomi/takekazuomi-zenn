...
ここに逆転が起きている。従来のレビューは、書き手のメンタルモデルが存在することを前提に、それをチームへ転送する営みだった。AI生成コードでは、そもそも書き手（AI）にメンタルモデルがない。Vogelsの言う「検証負債」[^7]とはこのことだろう。自分でコードを書けば理解は創作に伴うが、機械が書いたコードでは理解をゼロから再構築しなければならない。

しかも、AIはコードの生成速度を劇的に上げた。人間の理解速度は変わらない。この非対称性が累積すると、コードベースの中に「誰もメンタルモデルを持たない領域」が広がっていく。Naurの言う「死んだプログラム」の量産だ。

だから、AI生成コードの検証はまずPR作成者自身の責務になる。「AIが書いたから中身はよくわからないが、テストは通っている」。この状態でレビューに出すのは、レビューの前提を欠いている。提出するコードは作成者のメンタルモデルの中になければならない。AIが生成したコードを読み、修正し、自分のものにする。その上でチームに共有する。この手順を省略すれば、ブラックボックスが蓄積するだけだ。
...
実際、AI生成コードを最も大量に扱っている企業群がこの手順を省略していない。GoogleではAI生成コードが新規コードの25%を超えるが[^6]、すべてエンジニアのレビューと承認を経てコミットされる。コードレビューを廃止した大手テック企業は一社もない。むしろ、AIの普及がレビューの重要性を再確認させている。
