---
title: "関数の「色」問題：Go 1.26とC#の並行処理設計の分岐点"
emoji: ""
type: "tech"
topics: []
published: false
...
TODO: この２つは本質的に同じものである。**async/await方式**は、**コールバック/Promise方式**のシンタックスシュガーに過ぎない。と書く。
...
Cory Benfield（PythonのHTTPライブラリ群（urllib3、hyper-h2）の開発者）もNystromに対する反論を公開し、関数の色は「非同期性をプログラマに可視化する」ために時として良いトレードオフであると述べた。Arthur O'Dwyer（C++）はさらに踏み込んで、関数の「色」は型システムの自然な拡張であり、返り値の型や例外の有無と本質的に同じだと論じた。

### ConfigureAwait問題の現状

ASP.NET Coreは**初期バージョン（1.0）から** `SynchronizationContext`を持たない設計を採用しており、サーバーサイドでは`ConfigureAwait(false)`を至る所に書く必要がない。Stephen Clearyが2017年の記事で既に説明したように、これはASP.NET Coreの設計方針によるものであり、.NET 8で初めて起きた変化ではない。しかしデスクトップUI（WPF、WinForms）やXamarin/MAUIでは依然として問題が残り、ライブラリ作者は両方のコンテキストで動作するコードを書くために`ConfigureAwait(false)`を付ける慣習が続いている。

さらに注目すべき動きとして、.NET 11 Preview 1（2026年2月）で**Runtime Async**が導入された。これはasync/awaitをコンパイラのステートマシン変換からランタイムの第一級概念に昇格させる変更であり、コールスタックにasyncメソッドが直接表示され、ローカル変数のヒープ割り当ても削減される。本記事で「C#はコンパイル時にステートマシンに変換する」と述べた前提が変わりつつあり、C#のasync/awaitは「コンパイラの魔法」から「ランタイムの基盤機能」へと進化する方向にある。関数の色問題自体は残るものの、ステートマシン変換に伴うオーバーヘッドや診断困難性といったトレードオフが軽減されることになる。
...
| **Haskell** | 軽量スレッド（GHC RTS） | M:Nスケジューリング。`forkIO`でスレッド起動。IOモナドは関数の色問題そのものだが、型システムの設計原則として積極的に受け入れている |
...
| 言語 | 導入年 | 備考 |
|------|--------|------|
| **C#** | 2012（C# 5.0） | 先駆者。`Task<T>`/`ValueTask<T>`。ASP.NET Coreではサーバー側のConfigureAwait問題なし |
...
2023年にJava 21がProject Loomの仮想スレッドをGA（General Availability）として出荷したことは、関数の色問題に対する業界の態度の転換を象徴している。Javaは世界で最も広く使われているサーバーサイド言語の一つであり、その設計チームがasync/awaitを明示的に拒否してGoスタイルの仮想スレッドを選んだことは重い。仮想スレッドのAPI設計は意図的に既存の`java.lang.Thread`と互換であり、既存のブロッキングコードをそのまま仮想スレッド上で実行できる。Netflixは仮想スレッドへの移行で劇的なスケーラビリティ向上を報告している（ただしデッドロック問題も発見し、JDK 24（JEP 491）で修正された）。
...
1978  Tony Hoare「Communicating Sequential Processes」論文
       └→ CSP理論の確立
           │
1983  Occam言語（Hoare の CSP を直接実装、INMOS）
           │
1989  Rob Pike「Newsqueak」（Bell Labs）
       └→ Alef（Plan 9上, 1992頃）→ Limbo（Inferno OS, 1995）
       └→ CSPの実践的研究
           │
...
2026  Go 1.26: Green Tea GC デフォルト化、cgo 30%高速化
       ...
       └→ 「色なし」方式のランタイム品質がさらに向上
           │
2026  .NET 11 Preview 1: Runtime Async
       └→ async/await をランタイムの第一級概念に昇格
       └→ 「色あり」方式の実装品質がさらに向上
```
...
C#の方式は「非同期性の明示化と型安全性」を最適化する。Terry Crowleyが主張したように、非同期操作と同期操作の区別は現実世界の根本的な性質を反映しており、これを型レベルで表現することには設計上の価値がある。UIフレームワーク、分散システム、あるいはRustのようにCとの密接な統合が必要な領域では、この明示性が重要となる。さらに.NET 11のRuntime Asyncに見られるように、C#陣営はステートマシン変換のオーバーヘッドや診断困難性といった弱点をランタイムレベルで解消する方向に動いており、async/await方式も進化を続けている。

業界全体を見ると、Java 21の仮想スレッド、Ruby 3.0のFiber Scheduler、KotlinのElizarovの「理想的にはsuspendを排除したい」という発言など、「色なし」方向への関心は高まっている。一方で、C#のRuntime AsyncやRustのゼロコスト抽象のように、async/await方式の内部実装を改善することで色問題の実務上の痛みを軽減するアプローチも進行している。RustやC++ではC ABIとの互換性が不可欠であり、async/await方式が構造的に不可避である。この二つの潮流は対立するというよりも、それぞれの制約条件に応じた最適解を追求しているのだろう。
...
| Java | JEP 491: Synchronize Virtual Threads without Pinning | https://openjdk.org/jeps/491 |
| C# | Stephen Cleary「ASP.NET Core SynchronizationContext」(2017) | https://blog.stephencleary.com/2017/03/aspnetcore-synchronization-context.html |
| C# | .NET 11 Preview 1: Runtime Async | https://devblogs.microsoft.com/dotnet/dotnet-11-preview1/ |
